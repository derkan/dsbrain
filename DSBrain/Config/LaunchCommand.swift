...
enum LaunchCommand {
    ...
    /// Directory of the absolute-path binary (first token). ds4-server loads
    /// Metal sources like `metal/flash_attn.metal` relative to cwd.
    static func workingDirectory(from command: String) -> String? {
        guard let binary = tokenize(command).first else { return nil }
        let path = PathExpanding.normalize(binary)
